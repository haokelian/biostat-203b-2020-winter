...
admission_time <- 
  select(admission, ADMITTIME:DISCHTIME, starts_with("ADMISSION"), 
...
admission_time <- mutate(admission_time,
  ...
  hour = hour(ADMITTIME),
  days_of_hospital_stay = (unclass(as.POSIXct(DISCHTIME))- unclass(as.POSIXct(ADMITTIME)))/86400,
) %>% print(width = Inf)
