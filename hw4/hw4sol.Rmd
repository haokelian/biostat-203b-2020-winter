...
Add the severity of patients’ ailments by count the number of APR(All Payers Registry) included in the `DRG` codes in `drgcodes` table and pull the drug severity information and right-join it to our query table.
```{r}
tbl(con, "drgcodes") %>%
...
study_admissions_ = study_admissions
study_admissions_$drg_severity <- factor(study_admissions_$drg_severity)
study_logit <- glm(mortality ~ age + drg_severity + los, 
                   data = study_admissions_, 
                   family = "binomial")
(summary(study_logit))
```
...
```{r}
testdata1 <- with(study_admissions_, data.frame(age = rep(seq(from = 0, 
                                                                    to = 100,
                                                            length.out = 100),4
                                                         ),
                                                 los = mean(los), 
...
### Prepare data
Aquire data and separate the data set to training and test data.
```{r}
study_admissions$mortality = as.integer(study_admissions$mortality)
x_matrix = as.matrix(select(study_admissions, age, los, drg_severity))
y_matrix = as.matrix(select(study_admissions, mortality))
x_train = x_matrix[2:1000, ]
x_test = x_matrix[1000:1209, ]
dim(x_train)
dim(x_test)

y_train = y_matrix[2:1000]
y_train <- array_reshape(y_train, c(999, 1))
y_test = y_matrix[1000:1209]
y_test <- array_reshape(y_test, c(210, 1))
dim(y_train)
dim(y_test)
```
...
model %>% 
  ...
  layer_dense(units = 2, activation = 'softmax', input_shape = c(3))
...
```
The loss of data is high. The possible reason is the epoch is higher than enough, so that the model is over-trained. However, after several tries to cut down the loss by decreasing the epoch, it ends up with higher data loss.

### Generate predictions on new data:
```{r}
model %>% predict_classes(x_test) %>% head()
```
0 value in the prediction means that none of these patients will be died in 30 days after hospital discharge.

## Conclusion

In the beginning of this research, it is assumed that there are multiple factors that influence the 30-day mortality rate of patients after hospital discharge: age and gender of patients, insurance type, drug severity information, the specific disease, whether myocardia infraction is the principle disease or not, length of ICU stay are discussed to simulate the possible 30-day mortality rate of a myocardia infraction patient with certain features. However, according to the visualiztion of data, durg serverity information and age of patients and the length of ICU stay are selected as main factors of 30-day motality rate after hospital discharge, because different value of these variable leads to different 30-day mortality rates. The result of this simulization is that:

-Increasement of the patient's age increases the 30-day motality rate by 1.07 times.
-Increasement of length of ICU stay increases the 30-day motality rate by 1.10 times.
-Having a drug severity of 2, versus drug severity of 1, decreases the 30-day motality rate by 1.28.
-Neural network with multi-layer perceptron is about 86% accurate with about 50% loss of data.
